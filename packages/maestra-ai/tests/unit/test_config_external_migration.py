...
v0.11.0: getsongbpm removido; reccobeats adicionado.
migrate_external_sources migra automaticamente keys em plaintext para o keyring
(best-effort) e converte config legado getsongbpm → reccobeats.
"""
...
def test_source_enabled_helper_reads_nested():
    cfg = {"external_sources": {"musicbrainz": {"enabled": True}, "lastfm": {"enabled": False}, "reccobeats": {"enabled": False}}}
    assert cfg_module.source_enabled(cfg, "musicbrainz") is True
    assert cfg_module.source_enabled(cfg, "lastfm") is False
    assert cfg_module.source_enabled(cfg, "reccobeats") is False
...
def test_any_source_enabled_helper():
    cfg = {"external_sources": {"musicbrainz": {"enabled": False}, "lastfm": {"enabled": True}, "reccobeats": {"enabled": False}}}
    assert cfg_module.any_source_enabled(cfg) is True

    cfg2 = {"external_sources": {"musicbrainz": {"enabled": False}, "lastfm": {"enabled": False}, "reccobeats": {"enabled": False}}}
    assert cfg_module.any_source_enabled(cfg2) is False


def test_any_source_enabled_reccobeats():
    cfg = {"external_sources": {"musicbrainz": {"enabled": False}, "lastfm": {"enabled": False}, "reccobeats": {"enabled": True}}}
    assert cfg_module.any_source_enabled(cfg) is True


def test_migrate_getsongbpm_legacy_active_activates_reccobeats(monkeypatch):
    """GSB ativo no config legado → reccobeats ativado silenciosamente."""
...
def test_migrate_getsongbpm_legacy_inactive_does_not_activate_reccobeats(monkeypatch):
    """GSB desativo no config legado → reccobeats permanece False."""
    from unittest.mock import MagicMock
    mock_kr = MagicMock()
    monkeypatch.setattr(cfg_module, "keyring", mock_kr)
